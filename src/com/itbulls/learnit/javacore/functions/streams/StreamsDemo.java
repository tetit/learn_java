...
public class StreamsDemo {
	...
	public static void main(String[] args) {
		System.out.println(
				"============== filter() & map() & collect() demo ==========");

		List<Product> products = new ArrayList<>(Arrays.asList(new Product(
				"iPhone", 1099.99, 1345), new Product("Boots", 49.99, 15200),
				new Product("HP Laptop", 2599.99, 876), new Product("SSD",
						239.99, 900), new Product("Sony PlayStation", 2009.99,
								1001)));

		List<Product> modifiedProducts = products.stream().filter(
				product -> product.getItemsInWarehouse() > 1000).map(
						product -> {
							product.setPrice(product.getPrice() - (product
									.getPrice() * 0.1));
							return product;
		...
		warehouse1.setProducts(Arrays.asList(new Product("iPhone", 1099.99,
				1345)));
